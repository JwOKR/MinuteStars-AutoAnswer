# Changelog

## v4.9.57
### 🧹 清理
- **移除预设功能** — 删除预设方案 UI 和相关代码（约 100 行）

## v4.9.56
### ✨ 新增
- **内置预设方案** — 添加三个内置策略预设
  - 快速模式：关闭模糊匹配，答题间隔 50ms
  - 精准模式：模糊匹配阈值 0.85，答题间隔 120ms
  - 安全模式：模糊匹配阈值 0.90，答题间隔 300ms

## v4.9.55
### 🔧 优化
- **深色模式 CSS 合并** — 删除 `@media (prefers-color-scheme: dark)` 中重复的标题栏样式，保留 CSS 变量
- **登录检测优化** — `setInterval(1000)` 改为 `MutationObserver`，减少无效 DOM 查询

## v4.9.54
### 🧹 清理
- **删除废弃代码** — 移除未使用的 `@match` 域名、`@connect api.github.com`、`cloudGistId`
- **删除未使用方法** — 移除 `LZString.compressJSON`/`decompressJSON`、`idbGet`
- **删除无效代码** — 移除空的 `@media (light-level:...)` 规则、非标准 `pointerclick` 事件
- **优化日志** — `uLog` 的 console.log 绑定到 `CFG.debug` 开关

## v4.9.53
### 🔧 重构
- **常量集中管理** — 提取 `THRESHOLDS` 对象，统一管理所有魔法数字
  - 语义去重阈值、模糊匹配阈值、缓存大小、分页大小等
  - 消除代码中散落的硬编码数字

## v4.9.52
### 🐛 修复
- **批量导入自动同步** — 批量导入后自动同步到云端（需开启云同步并填写 Token）

## v4.9.51
### 🐛 修复
- **fetchCloudDB 统一** — 直读云端函数 `fetchCloudDB` 改用 `_fetchCloudDB`，修复解密失败返回 0 条的问题
- **错误提示增强** — 解密失败时显示具体排查步骤

## v4.9.50
### 🐛 修复
- **加密状态检测** — `_fetchCloudDB` 检测内容仍为 'ENC:' 开头时，明确提示解密失败并返回空对象

## v4.9.49
### 🐛 修复
- **云端读取容错** — `_fetchCloudDB` 处理文件不存在/为空/JSON解析失败的情况，返回空对象
- **loadCloudDB 统一** — 使用 `_fetchCloudDB` 替代手动解析，保持一致性

## v4.9.48
### 🔧 重构
- **云端同步重构** — 提取 `_mergeDatabases` 和 `_fetchCloudDB` 公共函数，消除三函数重复逻辑
- **源追踪修复** — 下载/导入时正确标记题目来源为 'cloud'
- **日志优化** — 统一显示 本地/云端/合并/新增 数量

## v4.9.47
### 🐛 修复
- **collectAnswers async 修复** — 函数声明改为 async，forEach 改为 for 循环，await 生效
- **采集查找性能优化** — O(n×m) 线性遍历改为 cleanMap O(1) 查找

### 🔧 重构
- **状态机去重** — 提取 `saveCurrentQA()` 函数，消除4处重复代码（~80行）

## v4.9.46
### 🔧 优化
- **采集后自动同步** — 采集答案后自动同步到云端（需开启云同步并填写 Token）

## v4.9.45
### 🐛 修复
- **多选题答案采集** — 修复多选题只采集到第一个字母的问题（如 "ABC" 只取到 "A"）
  - 现在正确采集 "ABC" → "A,B,C"

## v4.9.44
### ✨ 新增
- **采集调试增强** — 采集答案时记录详细日志
  - 显示每题使用的采集策略（reference-badge / input-mark / text-search）
  - 未找到答案时显示警告
  - 有失败题目时弹出调试面板，可查看HTML结构
- **策略三：文本匹配** — 新增从页面文本中匹配"正确答案：X"格式

## v4.9.43
### ✨ 新增
- **扫描功能增强** — 点击「🔍 扫描」显示详细弹窗
  - 统计卡片：题目数、题库总数、可匹配数
  - 每道题详情：题目摘要、单选/多选、选项列表、匹配状态、答案
  - 新拟态风格 UI

## v4.9.42
### 🐛 修复
- **SHA 冲突重试** — 云端写入遇到 SHA 冲突时自动重试（最多 3 次），避免并发更新导致失败

## v4.9.41
### 🔧 优化
- **采集日志详情** — 采集答案时显示每条题目的题目和答案内容

## v4.9.40
### 🔧 优化
- **采集答案更新** — 采集时如果题目已存在但答案不同，自动更新答案（原先跳过）

## v4.9.39
### 🔧 优化
- **本地编辑自动同步** — 本地题库编辑后自动同步到云端（需开启云同步并填写 Token）

## v4.9.38
### ✨ 新增
- **编辑题目功能** — 题库浏览和云端题库均支持编辑题目和答案
  - 本地题库：点击 ✏️ 按钮编辑，保存到本地
  - 云端题库：点击 ✏️ 按钮编辑，直接保存到云端
  - 支持修改题目内容和答案

## v4.9.37
### ✨ 新增
- **云端题库管理** — 题库管理弹窗新增「☁ 云端题库」标签页
  - 直接查看云端题库内容，无需先下载到本地
  - 支持搜索、分页浏览云端题目
  - 支持单条删除云端题目
  - 支持批量选择、批量删除、批量同步到本地

## v4.9.36
### ✨ 新增
- **批量删除** — 题库浏览新增「批量」模式，支持勾选多条题目一键删除
- **搜索清除按钮** — 搜索框右侧添加 ✕ 清除按钮，快速清空搜索

### 🎨 优化
- **新拟态风格统一** — 下拉框、输入框全部改为新拟态样式（移除暗色 `#2a2a2a`）
- **统计标签修正** — 第四个统计卡片改为「本地题库」（原先重复显示"题库总数"）
- **随机模式翻页一致** — 随机排序结果缓存，翻页时不再重新打乱
- **移除无意义筛选** — 隐藏「全部题目/builtin/user」筛选器（已无内置题库概念）
- **格式示例优化** — 批量导入说明更清晰，标注支持的文件格式

## v4.9.35
### 🔧 变更
- **添加 GreasyFork 元数据** — 添加 `@description` 和 `@license MIT`，准备上传到 GreasyFork/OpenUserJS

## v4.9.34
### 🐛 修复
- **重置按钮导致设置关闭** — `$$('input')` 误伤面板内开关，改为排除 `#ata-panel` 内的 input

## v4.9.33
### 🔧 变更
- **快捷键默认关闭** — `shortcutsEnable` 默认值 `true` → `false`
- **系统通知默认关闭** — `notifyEnable` 默认值 `true` → `false`

## v4.9.32
### 🔧 变更
- **恢复提交延迟 20~30s** — v4.9.31 的页面计时器拦截已生效，无需缩短延迟

## v4.9.31
### 🔧 变更
- **拦截页面 5 秒自动提交** — 注入 `window.setTimeout` 拦截器，将延迟 4-6s 的提交类定时器延长至 60s
- **提交延迟默认缩短** — `submitDelayMin/Max` 从 20~30s 改为 1~3s，避免与页面自身提交冲突

## v4.9.30
### 🔧 变更
- **加密上传默认开启** — `cloudEncrypt` 默认值 `false` → `true`

## v4.9.29
### 🐛 修复
- **修复重置/暂停后立即提交** — `processQuestions` 被重置中断后 `running=false`，但 `runAutoAnswer` 仍未加判断就调用了 `handleSubmitCountdown` 启动倒计时。新增 `&& running` 条件检查

## v4.9.28
### 🔧 变更
- **取消自动答题的 3s 倒计时** — 检测到题目后立即开始答题，不再等待 3 秒

## v4.9.27
### 🔧 变更
- **取消手动设置加密密码** — 移除密码输入框和所有相关逻辑，统一使用内置密码 `1129`。`_getEncryptKey()` 硬编码返回 `1129`

## v4.9.26
### ✨ 新增/变更
- **内置加密密码改为 1129** — `_getEncryptKey()` 默认值从 `ATA_DEFAULT_KEY` 改为 `1129`，留空加密密码时自动使用

## v4.9.25
### 🐛 修复
- **换电脑/新环境获取不到云端题库** — Gitee raw 链接会 302 重定向到 `raw.giteeusercontent.com`，该域名未在 `@connect` 白名单中导致 GM_xhr 拒绝连接。新增 `@connect giteeusercontent.com`

## v4.9.24
### 🐛 修复
- **Edge 浏览器获取不到云端题库** — `_cloudReq` 只检测旧版 `GM_xmlhttpRequest`，Edge 版 Tampermonkey 可能只暴露 `GM.xmlHttpRequest`（命名空间 API），导致跨域请求不可用。现在同时支持两种 API 并给出明确错误提示

## v4.9.23
### 🐛 修复
- **加密上传仍栈溢出** — `_encryptContent()` 中 `btoa(String.fromCharCode(...combined))` 同样存在展开运算符爆栈，统一为 `_toBase64()` 分块编码

## v4.9.22
### 🐛 修复
- **Maximum call stack size exceeded** — 上传时 `String.fromCharCode(...Uint8Array)` 展开大题库导致栈溢出，改为 8KB 分块拼接

## v4.9.21
### 🐛 修复
- **ReferenceError: Cannot access 'logEl' before initialization** — `uLog()` 改为惰性 DOM 查询，避免 `const` TDZ 导致初始化阶段 `handleLogin()` 调用崩溃

## v4.9.20
### ✨ 新增
- **题目加密上传**：AES-GCM 加密，云端文件存为密文（`ENC:` 前缀），不可直接阅读
  - 加密开关 + 可选自定义密码
  - 兼容未加密文件：读取时自动检测，未加密文件正常解析

## v4.9.19
### 🔄 修改
- **「🗑 清空云端」改为写入空 JSON `{}`**：不再删除文件，改为覆盖写入空对象 `{}`，更安全、无需重建文件

## v4.9.18
### ✨ 新增
- **删除云端文件**：新增「🗑 删除云端」按钮，可删除 Gitee 仓库中的题库文件（需 Token + projects 权限）

## v4.9.17
### 🐛 修复
- **题库上传后变成乱码**：`_writeRepoFile` 旧式 `btoa(unescape(encodeURIComponent(...)))` 编码和 `_readRepoFile` API 回退路径 `atob()` 解码均无法正确处理 UTF-8 中文。改用 `TextEncoder`/`TextDecoder` 标准 API

## v4.9.16
### 🐛 修复
- **匹配不到答案**：`rebuildCache()` 的 `_rebuilding` 锁从未释放，导致第一次调用后所有后续缓存重建被跳过，`getMergedCache()` 返回空/旧数据 -> 所有题目匹配失败

## v4.9.15
### 🐛 修复
- **GM_xmlhttpRequest 被 @connect 阻止**：添加 `@connect gitee.com`（Tampermonkey 安全策略要求所有跨域请求的域名必须在 `@connect` 列表中声明）

## v4.9.14
### 🐛 修复
- **跨域 CORS 错误**：`_cloudReq` 对 `gitee.com` 请求改用 `GM_xmlhttpRequest`（绕过 CORS），同源请求保持 `fetch`

## v4.9.13
### 🐛 修复
- **自动登录彻底重写**：`$()` 改为 `document.querySelector`（不依赖页面 jQuery）；MutationObserver 10s 超时改为 `setInterval` 持续轮询
- 登录表单消失后自动重置 `_loginHandled`，支持 session 过期后重新自动登录

## v4.9.12
### 🐛 修复
- **上传题库到 Gitee 400 错误**：`access_token` 从 body 移到 URL 参数；新文件用 POST，已有文件用 PUT（Gitee API 要求）
- **sha is missing 错误**：`_writeRepoFile` 根据是否有 sha 自动选择 POST（创建）或 PUT（更新）

## v4.9.11
### 🐛 修复
- **raw URL 请求优化**：GET 请求不再带 `Content-Type` 头，避免触发 CORS 预检导致 Gitee raw 被拦截
- **Token 权限错误提示**：API 回退失败时识别 `no 'projects' scope`，提示用户重新生成带 projects 权限的 Token

## v4.9.10
### 🐛 修复
- 云端题库文件未创建时避免 401 错误：`_readRepoFile` 在无 Token 时不尝试 API 回退
- `fetchCloudDB` 区分"文件不存在"和"拉取失败"，前者用 warn 而非 error

## v4.9.9
### 🔧 调整
- 提交延迟默认从 40-120s 改为 20-30s

## v4.9.8
### 🔧 调整
- `cloudReadMode` 保存 fallback 从 `'local'` 改为 `'cloud'`，与 CFG_DEFAULT 一致
- 确保新用户/重置后默认直读云端答题

## v4.9.7
### 🔧 调整
- 题库默认路径从 `tiku.json` 改为 `minutestars_qa.json`

## v4.9.6
### 🐛 修复
- **修复"保存"按钮未真正持久化设置**（所有设置，不止自动登录）
  - `cfg-save` 点击处理中缺少 `saveCFG()` 调用，`applySettingsFromUI()` 只写内存未存 GM 存储
  - 页面刷新后配置回退到旧值，导致自动登录、模糊匹配等所有设置修改均不生效

## v4.9.5
### 🐛 修复
- **修复自动登录不生效**
  - 浏览器自动填充干扰：`if (user.value) return` 改为 `user.value !== CFG.username`，填充正确用户名后仍会点登录
  - SPA 导航不触发：添加 `MutationObserver` 监听登录表单 DOM 出现
  - 防重复：`_loginHandled` 标记防止重复触发
  - 新增账号密码空值检查，未配置时静默跳过

## v4.9.4
### 🐛 修复
- **修复 `pct is not defined`** — `updateStatsCards()` 调用在 AI 移除时被误删，补回调用并赋值 `pct`

## v4.9.3
### 🐛 修复
- **修复 `libCnt` / `ruleCnt` 未声明** — `processQuestions` 中使用了三个未声明的变量，导致 `ReferenceError: libCnt is not defined`

## v4.9.2
### ✨ 新增
- **补全设置面板缺失项**
  - 仓库路径（`cloudRepoPath`）— 可自定义 Gitee 仓库 owner/repo
  - 分支名（`cloudBranch`）— 可自定义仓库分支（默认 main）
  - 数据压缩（`compressEnable`）— LZ-string 压缩开关，减少存储占用
- **清理 AI 残留 HTML** — 删除设置面板中遗留的 AI 相关空元素和错误的 `</select>` `</div>` 标记

## v4.9.1
### 🐛 修复
- **修复 AI 模块移除残留 bug**
  - `runAutoAnswer` 缺失 `processQuestions` 调用，导致 `ReferenceError: ok is not defined`
  - `processQuestions` 内 AI 残留代码导致规则推断成功后错误标记为"未匹配"并 `skip++`
  - 让 `processQuestions` 返回 `{ ok, infer, skip }` 供调用方使用

## v4.9.0
### 🔥 重大变更
- **彻底移除 AI 辅助模块**
  - 删除所有 AI 相关函数（`aiMatch`、`aiValidateAnswer`、`aiAnalyzeWrongAnswer` 等）
  - 移除 AI 设置 UI（模型选择、API Key、Endpoint）
  - 移除 AI 统计显示（方法分布不再含 AI）
  - 移除 AI 键盘快捷键（Ctrl+Shift+A）
  - 移除策略预设中的 AI 选项
  - 清理 AI CSS 样式
  - 代码精简约 370 行

## v4.8.53
### ⚙️ 默认值调整
- **模糊匹配默认关闭**（新用户/重置配置后默认精确匹配，需手动开启）
- 自动答题默认开启（不变）
- 自动提交默认开启（不变）

## v4.8.52
### ✨ 改进
- **Excel 导入结果显示与 Word 导入统一**
  - 导出按钮文案：`📋 导出本次解析日志（N 条）`（含条数，与 Word 一致）
  - 导出按钮条件：基于 `debugLog` 条数判断（与 Word 一致）
  - 导出回调逻辑：直接输出 `debugLog` JSON（与 Word 一致）
  - 消息拼接方式、样式、顺序完全对齐
  - 清理重复的 setTimeout 闭合（修复语法错误）

## v4.8.51
### 🚀 新功能
- **Gitee 仓库文件替代 Gist**（云同步重构）
  - 题库存储在仓库文件中（如 `tiku.json`），不再使用 Gitee Gist
  - 读题库免 Token（raw 链接公开访问），写题库需 Token
  - 分享功能：生成时间戳快照文件，返回 raw.gitee.com 公开链接
  - 导入功能：支持粘贴完整 raw URL 或仓库文件路径
  - 设置 UI 更新：移除 Gist ID，新增题库路径配置

## v4.8.50
### 🐛 修复
- **Excel 导入日志导出按钮：彻底简化**
  - 只要 `added > 0 || skipped > 0` 就显示导出按钮，零依赖
  - 导出数据自动回退：debugLog → result 数据
  - 所有数组字段添加 `|| []` 防崩溃

## v4.8.47
### ✨ 新功能
- **Excel 导入也支持解析日志导出**
  - Excel 导入完成后同样显示「📋 导出本次解析日志」按钮
  - 日志记录每道题的导入/跳过详情（与 Word 导入一致）
  - 下载文件名为 `Excel解析日志_xxx.json`

## v4.8.46
### ✨ 新功能
- **解析日志增强：记录每道题的导入/跳过详情**
  - `IMPORT` 日志：记录每条新增题目的题干和答案
  - `SKIP` 日志：记录每条跳过题目的原因（重复题目/题干过短/缺少答案）
  - 重复题目同时记录旧答案和新答案
  - 导出的日志 JSON 现在包含完整的逐题导入明细

## v4.8.45
### ✨ 新功能
- **支持无题号格式的文档解析**
  - 状态机现在可以识别不带题号的题干（如直接以文字开头的题目）
  - `LOOKING_FOR_QUESTION` 状态：遇到非空、非选项、非答案的文本行也视为题干开始
  - `FOUND_ANSWER` / `FOUND_ANALYSIS` 状态：遇到无题号的题干也能正确保存当前题并开始新题
  - 适用于「题干 + 答案」交替排列、无 A. B. C. D. 选项的文档格式

## v4.8.44
### ✨ 新功能
- **导入完成后可导出解析日志**
  - 每次导入前清空调试日志，只保留本次导入的记录
  - 导入完成后显示「📋 导出本次解析日志」按钮
  - 点击按钮下载 JSON 格式的完整解析日志（含时间戳、级别、分类、消息、数据）
  - 方便排查导入问题和确认解析细节

## v4.8.43
### ✨ 新功能
- **支持 docx 多文件同时导入**
  - Word 文档导入按钮现在支持选择多个 .docx 文件
  - 批量处理所有选中的文件，汇总导入结果
  - 显示每个文件的解析进度（1/3、2/3、3/3）
  - 汇总显示：总导入数、总跳过数、重复覆盖数
  - 部分文件解析失败时显示具体错误，不影响其他文件继续导入

## v4.8.42
### ✨ 新功能
- **添加 Excel (.xlsx) 导入功能**
  - 新增「📊 从 Excel 导入（.xlsx）」按钮
  - 动态加载 SheetJS 库（无需预装）
  - 支持自动识别题目和答案列（表头包含"题目"/"问题"/"答案"/"answer"等关键词）
  - 默认第一列为题目，第二列为答案
  - 自动清理题号前缀、标准化答案格式
  - 支持去重检查和批量导入

## v4.8.41
### 🐛 Bugfix
- **修复导入题库刷新后丢失的问题**
  - 问题：`parseDocxDocument` 调用 `parseWithStateMachine` 后，虽然修改了 `db` 对象，但没有调用 `LibraryManager.save()` 持久化保存
  - 解决：在状态机解析完成后，调用 `LibraryManager.save(db)` 保存到存储
  - 效果：导入的题目现在可以正确持久化，刷新页面后不会丢失

## v4.8.40
### 🧹 清理
- **Phase 5: 代码清理 - 删除所有遗留的旧函数**
  - 删除 `extractDocxXMLLegacy` 函数（Phase 1 兼容用）
  - 删除 `extractContentBlocksLegacy` 函数（Phase 1 兼容用）
  - 删除 `parseDocxBlob` 函数（旧入口函数）
  - 删除 `loadJsZipAndParse` 函数（旧 JSZip 加载函数）
  - 删除 `extractParagraphs` 函数（旧段落提取函数）
  - 删除 `parseQAFromParagraphs` 函数（旧解析函数）
  - 删除 `parseDocxXML` 函数（旧 XML 解析函数）
  - 删除 `parseQAFromParagraphsNew` 函数（过渡期函数）
  - 新架构（`parseDocxDocument` + `extractDocxXML` + `extractContentBlocks` + `parseWithStateMachine`）已完全替代旧代码
  - **Docx 解析代码重构全部完成**

## v4.8.39
### 🔧 优化
- **Phase 4: 调试和优化**
  - 增强 `debugLog` 函数，添加日志级别控制（ERROR/WARN/INFO/DEBUG/TRACE）
  - 新增 `showDocxDebugLog` 函数（在控制台显示调试日志）
  - 新增 `clearDocxDebugLog` 函数（清空调试日志）
  - 改进 `parseDocxDocument` 错误处理，提供更友好的用户提示
  - 添加文件大小检查、XML内容检查、内容块检查
  - 在解析失败时附带调试日志，便于问题定位

## v4.8.38
### ✨ 新功能
- **Phase 3: 格式扩展 - 支持表格和页眉页脚**
  - 新增 `extractBlocksFromTable` 函数（从表格中提取文本内容）
  - 新增 `extractHeadersFooters` 函数（从页眉页脚中提取文本内容）
  - 新增 `extractBlocksFromElement` 函数（从元素中提取段落内容）
  - 新增 `extractTextFromElement` 函数（从元素中提取文本内容）
  - 更新 `extractContentBlocks` 函数，支持段落、表格、页眉页脚三种格式
  - 现在可以正确解析包含表格的 Word 文档中的题目

## v4.8.37
### 🐛 Bugfix
- **修复状态机解析多行内容块的问题**
  - 问题：`extractContentBlocks` 返回的内容块包含多行文本（用 `\n` 分隔），但状态机期望每个块是一行
  - 解决：在调用状态机之前，将多行内容块分割成单独的行
  - 效果：状态机现在能正确识别每个题目、选项、答案和解析行

## v4.8.36
### 🔄 重构
- **Docx 解析代码重构（Phase 2）：实现状态机解析逻辑**
  - 新增 `parseWithStateMachine` 函数（状态机版本解析器）
  - 实现完整状态机：`LOOKING_FOR_QUESTION` → `COLLECTING_QUESTION` → `COLLECTING_OPTIONS` → `FOUND_ANSWER` → `FOUND_ANALYSIS`
  - 状态机支持多行题干、选项收集、答案识别、解析跳过
  - 更新 `parseDocxDocument` 调用状态机版本（替代 `parseQAFromParagraphsNew`）
  - 保留 `parseQAFromParagraphsNew` 作为备用（向后兼容）
  - 状态机版本具有更清晰的逻辑流程和更好的可维护性

## v4.8.35
### 🔄 重构
- **Docx 解析代码重构（Phase 1/2）：新架构基础框架 + 代码清理**
  - 新增 `parseDocxDocument` 函数（新入口，替代 `parseDocxBlob`）
  - 新增 `extractDocxXML`（解压 docx 并提取 XML）
  - 新增 `extractContentBlocks`（解析 XML 为段落文本数组）
  - 重构 `parseQAFromParagraphsNew`（替代旧 `parseQAFromParagraphs`，提取 helper 函数，代码更清晰）
  - 新增调试日志函数 `debugLog`
  - 事件处理器更新为调用新函数
  - 旧代码暂保留（`parseDocxBlob`, `loadJsZipAndParse`, `extractParagraphs`, `parseQAFromParagraphs`），后续清理

## v4.8.33
### 🐛 Bugfix
- **extractParagraphs 彻底重写，正确提取 Word 段落内换行**
  - 根因：v4.8.32 用了 `p.childNodes`，但 `<w:t>` 嵌套在 `<w:r>` 里，根本扫不到，`allTexts` 提取结果为空
  - 解法：用 `p.getElementsByTagName('*')` 获取所有后代元素（按文档顺序）
  - 检查 `el.localName`：`t` → 取文本；`cr`/`br` → 插入 `\n`
  - 最终按 `\n` 分割，`parseQAParagraphs` 能正确识别题目/选项/答案/解析各成一行

## v4.8.32（废弃：实现有误，被 v4.8.33 替代）
### 🐛 Bugfix
- 尝试修复 extractParagraphs，但实际实现用了 `p.childNodes`，无法正确提取 `<w:t>` 文本

## v4.8.31
### 🐛 Bugfix
- **解析行拦截逻辑改为最宽泛匹配：只要行中包含"解析"二字，就不加入 qLines**
  - 内层循环：`if (/解析/.test(trimmed)) { break; }`
  - qText 清洗：`.filter(l => !/解析/.test(l))` 整行包含"解析"的直接丢弃
  - 双层保险，无论"解析"行格式如何（全角/半角/空格/换行）都不会混入题目

## v4.8.30
### 🐛 Bugfix
- **解析行不再加入 qLines，彻底阻断解析内容混入题目**
  - 遇到"答案："或"正确答案："行 → 收集该行后 break
  - 遇到"解析："行（无论行首还是行中）→ 不收集，直接 break
  - 配合 qText 清洗（移除答案/解析内容到行尾），双层保险

## v4.8.29
### 🐛 Bugfix
- **parseQAFromParagraphs 答案/解析行处理逻辑重写**
  - 遇到答案行或解析行后立即 `break`（不再使用 foundAnswer 标志 + continue 的两轮判断）
  - qText 清洗改用 `[^\n]*`（匹配到行尾），更激进地移除"答案/正确答案/解析/【解析】"内容
  - 新增支持"正确答案："格式的答案行
  - 新增匹配行中任意位置的"解析："（不仅限于行首），防止解析内容漏网

## v4.8.28
### 🐛 Bugfix
- **重写 parseQAFromParagraphs 内层循环，彻底防止解析内容混入题目**
  - 新增 `foundAnswer` 标志：找到答案行后，后续所有行（解析、解释列举等）一律停止收集
  - 解决题目、选项、答案、解析各自独立成行时，解析内容被误收入题目体的问题
  - 同时防御：答案行与解析行同处一段落时也能正确截断

## v4.8.27
### ✨ 优化
- **主面板「快捷键 & 通知」区域新增快捷键提示**
  - 在「启用快捷键」开关下方显示：`Alt+Enter` 开始答题/提交、`Alt+S` 暂停/继续、`Alt+D` 下载题库JSON

## v4.8.26
### ✨ 优化
- **主面板三个云同步按钮新增鼠标悬停提示（title）**
  - 「⬆ 上传题库（合并）」：提示"上传本地题库到云端 Gist，与云端已有题目合并去重，本地题目优先保留"
  - 「⬇ 下载题库（覆盖）」：提示"从云端 Gist 下载题库，覆盖本地所有题目（云端为唯一权威来源）"
  - 「☁ 导入云端」：提示"将云端 Gist 题库追加导入到本地（不影响云端，仅本地新增）"

## v4.8.25
### 🐛 Bugfix
- **修复 docx 导入时，解析内容的数字列举项（2.轻则... / 3.重则...）混入题目**
  - `parseQAFromParagraphs()` 内层循环判断"下一题/答案/解析"时，改用 `trimmed`（去空格后）做正则匹配
  - 原代码直接用 `nextLine` 匹配 `^\d+`，docx 提取行首带空格时匹配失败，"2.xxx" 被错误加入题目
  - `qText` 清洗阶段新增过滤：去掉以数字开头的行（防御性，防止解析列举项漏网）
- **修复外层循环题目行检测同样的问题**
  - `line.trim()` 后再做 `^\d+` 匹配，防止题目行因前导空格被跳过

## v4.8.24
### 🐛 Bugfix
- **修复 docx 文档导入题目时，题目分类标题被加入题目内容**
  - `parseQAFromParagraphs()` 收集题目行时，新增过滤"一、单选题"/"二、多选题"/"三、判断题"等分类标题（行首中文数字+顿号）
  - 同时过滤"第X部分"/"第X章节"格式的分类标题
- **修复 docx 文档导入题目时，解析内容（解析：...）残留在题目中**
  - `qText` 清洗时新增删除 `【解析】` 格式内容
  - 防止非常规格式解析内容未被正则匹配到

## v4.8.23
### ♻️ 重构
- **A: 拆分 runAutoAnswer()**
  - 提取 `processQuestions(containers, seenQ)` 函数（题目遍历、匹配、答题、日志全逻辑）
  - `runAutoAnswer()` 从 144 行缩减至 ~68 行
- **B: 拆分 renderBrowse()**
  - 提取 `buildBrowseRows(entries, page, keyword, useRegex)`（表格行构建）
  - 提取 `renderPager(total, page)`（分页控件更新）
  - `renderBrowse()` 从 81 行缩减至 ~58 行
- **C: 参数对象化**
  - `updateStatsCards(ok, infer, skip, i, total, libCnt, ruleCnt, aiCnt)`
    → `updateStatsCards({ ok, infer, skip, i, total, libCnt, ruleCnt, aiCnt })`
  - `updateHitRate(answered, hit, lib, rule, ai)`
    → `updateHitRate({ answered, hit, lib, rule, ai })`

## v4.8.22
### 🐛 Bugfix
- **恢复 v4.8.21 误注释的函数**
  - `debugScan`、`collectAnswers` 被事件监听器调用（`addEventListener('click', ...)`），注释后导致 `ReferenceError`
  - 已恢复全部 6 个函数：`_idleWrap`、`aiValidateAnswer`、`aiAnalyzeWrongAnswer`、`doSave`、`collectAnswers`、`debugScan`
  - v4.8.21 的"死代码"判断有误，这些函数均有调用点

## v4.8.21
### 🧹 清理
- **注释死代码（先观察再决定是否删除）**
  - `_idleWrap`（原行 990）
  - `aiValidateAnswer`（原行 1368）
  - `aiAnalyzeWrongAnswer`（原行 1419）
  - `doSave`（原行 3895）
  - `collectAnswers`（原行 3946）
  - `debugScan`（原行 4034）
  - 共注释 225 行，若观察期无异常可正式删除

## v4.8.20
### ♻️ 重构
- **拆分 parseDocxXML()**
  - 改为 `async function`，内部调用已提取的 `parseQAFromParagraphs(allTexts, db)`
  - 修复 bug：原函数未声明 `db`/`added`/`skipped`/`preview`/`duplicates` 直接使用（会导致 ReferenceError）
  - 现在正确通过 `await LibraryManager.load()` 获取 `db`，调用 `parseQAFromParagraphs` 处理，再 `LibraryManager.save(db)` 保存
  - 函数从 ~87 行缩减至 ~15 行

## v4.8.19
### ♻️ 重构
- **拆分 aiMatch()**
  - 提取 `buildAIPayload(qText, optTexts)` 函数（按模型构建请求体/请求头）
  - 提取 `doAIRequest(url, headers, body)` 函数（执行 XHR 请求，非 200 时 reject）
  - 提取 `parseAIResponse(resp)` 函数（按模型解析 AI 响应，返回 `{status,result}`）
  - 主函数从 142 行缩减至约 60 行，重试/错误处理逻辑更清晰

## v4.8.18
### ♻️ 重构
- **拆分 runAutoAnswer()**
  - 提取 `handleSubmitCountdown(minS, maxS)` 函数（提交倒计时逻辑）
  - 提取 `updateStatsCards(ok, infer, skip, i, total, libCnt, ruleCnt, aiCnt)` 函数（统计卡片实时更新）
  - 主流程函数从 183 行缩减至约 120 行

## v4.8.17
### 🐛 修复
- **修复云端模式来源统计错误（显示全为本地题库）**
  - 根因：`getSourceStats()` 调用 `this.load()`，云端模式下返回的是已合并云端的缓存，`localDB` 包含了云端题，导致 `stats.cloud` 始终为 0
  - 修复：`getSourceStats()` 改为直接读 `GM_getValue(DB_KEY)` 原始本地数据，绕过合并缓存
- **修复拉取云端后 UI 不更新**
  - `fetchCloudDB()` 成功后新增调用 `refreshLibCount()` 和 `refreshStats()`

## v4.8.16
### 🐛 修复
- **修复云端模式页面加载时不自动读取云端题库**
  - 根因：`fetchCloudDB()` 只在「保存设置」时调用，页面初始化（IIFE）时没有触发
  - 在初始化流程（第5058行附近）加入：`if (CFG.cloudReadMode === 'cloud') { fetchCloudDB(); }`
  - 确保页面加载后云端题库立即开始拉取

## v4.8.15
### 🐛 修复
- **修复分享到 Gitee Gist 失败（HTTP 400 Bad Request）**
  - POST 创建 Gist 时 payload 缺少 `description` 字段，Gitee API 要求必须提供
  - 加上 `description: 'MinuteStars 题库分享（${count} 题）'`

## v4.8.14
### 🐛 修复
- **修复点击「导入分享」tab 报错 `Cannot read properties of null (reading 'classList')`**
- 补上缺失的 `pane-import-shared` HTML（之前只有 tab 没有对应 pane，导致 `$('#pane-import-shared')` 为 null）
- 新增「从分享链接导入题库」面板：输入 Gist ID → 点击导入
- `cloudImport()` 支持传入自定义 `gistId` 参数（不再只读取 `CFG.cloudGistId`）

## v4.8.13
### 🐛 修复
- **修复云端模式无法匹配题目**：`rebuildCache()` 云端模式下只用了 `_cloudCache`，没有合并本地题库
- 修复：`userDB = { ...localDB, ...(_cloudCache || {}) }`（本地 + 云端合并，云端优先）
- 修复因 `_cloudCache` 为 `null` 时 `userDB = {}` 导致完全匹配不到题目的问题

## v4.8.12
### 🐛 修复
- **修复题库管理面板显示位置异常**：恢复正确的居中浮窗样式
  - `#ata-lib-modal` 改为 `position:fixed; top:50%; left:50%; transform:translate(-50%,-50%)`
  - 移除错误的覆盖层样式（`top:0;left:0;right:0;bottom:0`）
  - 删除孤立的 CSS 块（无选择器的样式代码）
  - 添加 `#ata-lib-modal.show { display:flex !important; }` 控制显示/隐藏

## v4.8.11
### 🐛 修复
- **修正题库来源定义**：按用户定义重新划分来源
  - 本地题库 = 本地存储里的题（本地导入 + 从云端下载到本地的）
  - 云端题库 = 直读云端且不在本地存储的题（仅 cloudReadMode 时存在）
- `cloudDownload()`：下载到本地的题改为标记 `'local'`（之前错误标记为 `'cloud'`）
- `cloudImport()`：导入到本地的题改为标记 `'local'`
- `getSourceStats()`：重写统计逻辑，正确区分本地存储题和直读云端题

## v4.8.10
### 🐛 修复
- **修复刷新后本地导入题库丢失问题**：`save()` 和 `reload()` 现在同时备份数据到 `GM_setValue`
- 确保 `load()` 在 `_cache.raw` 为 null 时能立即从 `GM_getValue` 读到数据，无需等待异步 `reload()` 完成
- 修复因数据存进 IndexedDB 后，`GM_getValue` 读不到导致刷新后题库消失的问题

## v4.8.9
### 🐛 修复
- **修复 `_cache is not defined` 错误**：添加 `_cache` 变量初始化（`raw/dirty/userCount/dirtyKeys/cleanMap/ngramIndex/lenBuckets`）
- 修复导致 `ReferenceError: _cache is not defined` 的脚本崩溃问题

## v4.8.8
### 🐛 修复
- **修复脚本语法错误**：删除第 2895 行孤立的 `l);`，添加正确的 `document.body.appendChild(modal);`
- 修复导致脚本无法运行的 `Uncaught SyntaxError: Unexpected token ')'` 错误

## v4.8.7
### 🐛 修复
- **修复题库来源显示错误**：本地导入的题库不再显示为云端题库
- 新增题目来源追踪（`_sourceMap`），记录每道题的来源（cloud/local）
- 修复 `refreshStats()` 饼图硬编码 100% 云端的问题，改为按实际来源分布计算

## v4.8.6
### ✨ 新功能
- **支持无 Token 匿名读取公开 Gist**：下载/导入题库不再强制要求 Token，只需填写 Gist ID（Gist 需设为公开）
- **Token 输入框提示更新**：placeholder 改为"Gitee 私人令牌（仅上传需要）"，并增加说明文字

### 🔧 优化
- `fetchCloudDB()`、`cloudDownload()`、`cloudImport()` 移除 Token 强制校验，匿名读取公开 Gist

## v4.8.5
### ⚙️ 配置调整
- **答题模式默认改为云端**：`cloudReadMode: 'local'` → `'cloud'`（直读云端答题，不落地）

## v4.8.4

### ⚙️ 配置调整
- **自动答题默认开启**：`autoAnswer: false` → `true`
- **自动提交默认开启**：`autoSubmit: false` → `true`

## v4.8.3

### ⚙️ 配置调整
- **云同步默认开启**：`cloudSyncEnable: false` → `true`（默认打开云同步）

## v4.8.2

### 🗑 移除功能
- **移除复习模式**：删除 WrongQuestionManager、复习模式弹窗及相关 UI
- **移除排行榜功能**：删除排行榜按钮及相关代码

### 🐛 修复
- 清理复习模式相关 CSS、HTML、JavaScript 代码
- 注释掉 WrongQuestionManager.add 调用
- 更新 @description，移除"错题本复习模式"描述

## v4.8.1

### ✨ 新功能（StorageManager 五大优化）
- **IndexedDB 自动迁移**：检测到大数据（>1MB）时自动从 GM_setValue 迁移到 IndexedDB
- **LZ-string 压缩支持（可选）**：新增 `compressEnable` 配置，启用后自动压缩存储数据（默认关闭）
- **数据版本管理**：StorageManager 内置版本号（VERSION: 2），支持未来自动迁移
- **存储统计信息**：新增 `getStorageInfo()` 方法，返回后端/条数/大小等统计

### 🔧 性能优化
- **IndexedDB 批量读写优化**：单次事务完成批量操作，减少 I/O 开销
- **游标读取优化**：`idbGetAll()` 使用游标批量读取，性能提升

### 🛡 错误处理增强
- **IndexedDB 可用性检测**：新增 `_checkIDBSupport()`，自动检测并降级到 GM_setValue
- **降级策略完善**：IndexedDB 失败时自动切换后端，确保数据不丢失
- **迁移失败处理**：迁移到 IndexedDB 失败时自动降级到 GM_setValue

### 🐛 修复
- 修复 `_openIDB()` 版本管理问题（硬编码版本号 → 使用 VERSION 常量）
- 修复 `idbSetAll()` 中未使用的 `processed`/`total` 变量

### 📝 配置新增
- `compressEnable: false`：压缩支持开关（默认关闭，避免性能影响）

## v4.8.0

### ✨ 新功能（Phase3）
- **多 AI 模型支持**：支持 DeepSeek（硅基流动）/ OpenAI（GPT-4o）/ Claude / Gemini 一键切换
- **每模型独立配置**：各模型可单独设置 API Key / Endpoint，互不干扰
- **LRU 匹配缓存**：缓存最近 500 条匹配结果，避免重复计算，提升答题速度

### 🔧 性能优化
- **匹配缓存**：LRU 缓存最近 500 条匹配结果，命中直接返回
- **多模型适配**：自动识别模型响应格式（OpenAI / Claude / Gemini）

### 🤖 AI 功能增强
- **多模型切换**：UI 新增模型选择器（DeepSeek / OpenAI / Claude / Gemini）
- **模型配置独立保存**：切换模型时自动加载对应 API Key / Endpoint
- **请求格式自适应**：根据模型自动构建正确请求（OpenAI 兼容 / Claude / Gemini）
- **AI 答案验证**：用 AI 验证题库答案的正确性（新增 `aiValidateAnswer`）
- **AI 错题分析**：分析错题原因并给出学习建议（新增 `aiAnalyzeWrongAnswer`）

## v4.7.0

### ✨ 新功能（Phase2）
- **错题本 + 复习模式（Spaced Repetition）**：艾宾浩斯简化版间隔 [1,3,7,15,30] 天，自动提醒复习
- **题库语义去重增强**：N-gram + Jaro-Winkler 检测，相似度 > 0.88 自动跳过入库
- **答题统计增强（正确率趋势图）**：Canvas 绘制最近 50 题命中率折线，实时更新
- **答案来源标注**：最近答题记录中显示彩色圆点（蓝=题库，黄=规则，绿=AI，灰=未匹配）

### 🔧 UI 优化
- 复习模式独立弹窗（📖 复习模式按钮），支持提交/跳过/从题库删除
- 正确率趋势图（📊 正确率趋势）Canvas 实时绘制，显示当前+平均正确率
- 答题记录来源可视化：彩色圆点标注每题匹配方式

## v4.6.0

### ✨ 重大升级：IndexedDB 大数据存储
- **StorageManager 抽象层**：根据数据大小自动选择后端（≤1MB 用 GM_setValue，>1MB 用 IndexedDB）
- **突破 5MB 瓶颈**：IndexedDB 理论无上限，彻底解决 GM_setValue 存储限制
- **向后兼容**：小数据量仍用 GM_setValue，无缝切换
- **LibraryManager 重构**：新增 `async reload()` 异步加载，`save()` 改为异步

### ✨ 新功能
- **域名通配符支持**：`@match` 改为 `*://*.minutestars.com/*`，自动匹配所有子域名
- **自定义域名配置**：`customDomains` 配置项，支持添加任意匹配域名
- **AI 请求去重缓存**：同题只问一次，用 Map 缓存避免重复 API 调用
- **AI 指数退避重试**：429 频率限制后自动重试（1s, 2s, 4s）
- **AI 连续失败自动禁用**：连续失败达到阈值自动关闭 AI 开关

### 🔄 架构优化
- **初始化改为 async IIFE**：确保题库异步加载完成后再开始答题
- **rebuildCache 改为同步**：从内存缓存读取，避免调用链断裂
- **StorageManager API**：统一 `get/set/remove`，内部自动选择最佳后端

### 🐛 Bug 修复
- 修复多次编辑后文件内容偏移导致 Edit 失败的问题（改用 Write 工具）
- 修复 AI 匹配函数入口参数处理不完整的问题

## v4.5.66

### ✨ 新功能：直读云端模式
- 新增 `cloudReadMode` 配置：切换「📥 本地存储」和「☁ 直读云端」两种答题模式
- **直读云端模式**：答题时直接从 Gist 拉取题库，**不写入本地存储**，内存缓存 5 分钟自动过期
- 切换模式后自动重新拉取云端题库并重建索引
- 启动时若为直读云端模式，自动预拉取云端题库

## v4.5.65

### 🐛 Bug Fix
- 修复语法错误：`const BUILTIN_DB = {};` 后的孤立键值对残留导致 `SyntaxError: Unexpected token ':'`

## v4.5.64

### 🔄 架构升级：纯云端模式
- **移除内置题库**：内置题库（BUILTIN_DB）已清空，不再内置固定题库
- **纯云端题库**：答题题库完全来自用户自定义（本地 GM_storage 或云端 Gist）
- **上传合并去重**：`⬆ 上传题库` 新增合并逻辑——自动获取云端现有题库，与本地合并后上传（本地答案优先），重复题目不会覆盖本地
- **下载完全覆盖**：`⬇ 下载题库` 改为云端完全覆盖本地（云端为唯一权威来源）
- **新增导入云端**：`☁ 导入云端` 新增按钮——获取云端题库，追加到本地（本地已有题目保留，云端补充缺失条目）
- **统计与浏览适配**：移除内置/自定义分类筛选，统一显示本地题库数量

## v4.5.63

### 🐛 Bug 修复（3项）
- **rebuildCache 空值保护**：`dirtyKeys?.size` 判空，防止 `Cannot read properties of null (reading 'size')`
- **rebuildCache 全量路径保护**：`dirtyKeys?.clear()` 防止 `Cannot read properties of null (reading 'clear')`
- **clear() 行为修正**：清空题库后改用 `dirtyKeys?.clear()` 而非 `=null`，保持 Set 实例避免后续 rebuildCache 空指针

## v4.5.62
- **clear() 行为修正**：改用 `.clear()` 而非 `=null`，保持 dirtyKeys 始终为 Set 实例
- **CSS语法修复**：`.ata-presets-row`/`.ata-presets-hint` 错误嵌套在 `.ata-collapse-body` 内部，已提取为独立规则
- **事件重复绑定**：删除 `#ata-clear-lib/yes/no` 第一组重复绑定，保留带空值检查的 `$c()` 版本
- **Canvas CSS变量**：`fillStyle` 不支持 `var(--nm-text)`，改为具体颜色 `#5a6a7a`
- **_speedTimes内存膨胀**：超过200条后 `slice(-200)` 截断
- **runAutoAnswer状态清理**：用 `finally` 块统一清理，排除 autoSubmit 倒计时场景
- **cleanText正则**：改用 Unicode 属性 `\p{P}\p{S}` 替代手动列举标点

### ⚡ 性能优化（4项）
- **cleanText缓存**：加 `_cleanTextCache` Map，相同文本直接返回缓存，上限2000条
- **rebuildCache增量更新**：新增 `dirtyKeys` 集合追踪变更，<50条时只更新变更部分
- **findQContainers DOM缓存**：首次查询后缓存，后续调用检查 `isConnected` 直接返回
- **aiMatch错误处理**：区分 401 Key无效、429频率限制、超时、网络失败等具体错误

## v4.5.61

### 🎨 UI 优化
- 深色模式：在 `prefers-color-scheme: dark` 媒体查询中添加标题栏覆盖样式

## v4.5.60

### 🎨 UI 优化
- 深色模式标题栏：使用 `style.setProperty` + `!important` 强制覆盖 CSS

## v4.5.59

### 🎨 UI 优化
- 深色模式检测：同时检测 documentElement 和 document.body 背景色
- 深色模式标题栏：通过 JavaScript 直接设置背景色（更可靠）

## v4.5.58

### 🎨 UI 优化
- 深色模式标题栏：添加 `!important` 强制覆盖原有样式

## v4.5.56

### ✨ 功能优化
- 最近答题记录改为显示当前试卷**全部试题**（不再限制10条）
- 每次答题开始时自动清空记录，确保只显示当前试卷

## v4.5.55

### 🐛 Bug Fix
- 修复 `_speedTimes` 未定义错误：将答题速度记录变量提升到外层作用域

## v4.5.54

### 🐛 Bug Fix
- 修复变量重复声明错误：`settingsBody` 被声明两次

## v4.5.53

### 🐛 Bug Fix
- 修复语法错误：删除孤立代码块（多余闭合括号导致 `Unexpected identifier '$$'`）


### ✨ 主面板大升级
- 📊 **实时命中率显示**：进度条实时显示命中率百分比，带颜色渐变
- 📋 **最近答题记录**：折叠区域显示最近10题，点击可查看详情弹窗
- 🏷️ **题库标签管理**：新建/删除标签，给题目分配标签，按标签筛选
- ⚙️ **答题策略预设**：内置快速/精准/安全三种预设，支持保存自定义预设
- 🔍 **设置项搜索**：输入关键词快速定位设置项
- 🌙 **深色模式适配**：自动检测页面背景色，面板自动切换明暗主题
- 📈 **答题速度曲线**：折线图实时显示答题速度趋势
- 🥧 **题库来源分布饼图**：统计面板显示内置/自定义题库比例
- ⌨️ **键盘导航支持**：设置区 Enter 确认，Esc 关闭弹窗
- 📱 **移动端适配增强**：小屏幕自适应布局

## v4.5.51

### 🔥 简化
- 移除 GitHub Gist 云同步和代理功能，仅保留 Gitee Gist 云同步

## v4.5.50

### 🆕 新增域名支持
- 自动登录和答题功能扩展支持以下域名：erp.minutestars.com / marketoperation.minutestars.com / multimedia.minutestars.com / zhibo.minutestars.com

## v4.5.49

### 🆕 新功能
- 新增 Gitee Gist 云同步支持（国内用户首选，无需代理）
- 设置面板新增「云平台」切换（Gitee / GitHub）
- GitHub 模式下仅在选择时才显示代理地址字段

### 🐛 Bug 修复
- 重构 `_gistReq` 支持自定义 headers
- 重构 `gistBody` 统一处理两个平台的 API 差异

## v4.5.46

### 🐛 Bug 修复
- 添加 `@connect api.github.com` 指令，让 Tampermonkey 明确放行 GitHub API 请求

## v4.5.45

### 🐛 Bug 修复
- 调整 `_gistReq` 请求优先级：fetch 优先、CORS 拦截时自动降级到 GM_xmlhttpRequest
- 改进 CORS 错误检测与自动切换逻辑

## v4.5.44

### 🐛 Bug 修复
- 修正 GitHub API Authorization Header 格式为 `token <token>`（GitHub 官方推荐格式）
- 移除多余的 `Accept` header，确保请求头精简
- 改进 GM_xmlhttpRequest 错误日志

## v4.5.43

### 🐛 Bug 修复
- 重构 Gist 请求：优先使用原生 `fetch` API（跨域支持更好），`GM_xmlhttpRequest` 作为兜底

## v4.5.42

### 🐛 Bug 修复
- 增强 Gist 请求错误日志：详细输出 HTTP 状态码、响应体前200字符、Token 前6位
- 修正 Authorization Header 为标准 `Bearer Token` 格式

## v4.5.41

### 🐛 Bug 修复
- 修复 GitHub Gist 上传 404 错误：添加 Gist ID 有效性验证，无效时自动清空重新创建新 Gist

## v4.5.40

### 🐛 Bug 修复
- 修复设置区折叠时保存按钮无法点击的问题（`max-height:0` + `overflow:hidden` 导致控件被裁剪，事件无法触发）
- `saveCFG` 增加错误日志输出，便于排查保存失败问题
- 修复 GitHub Gist 云同步网络错误：添加缺失的 `@grant GM_xmlhttpRequest` 声明

## v4.5.39

### 🔬 算法升级
- ⚡ Jaro-Winkler 算法替换 Levenshtein：对中文近形字（未/末、大/太）及开头相似的字符串匹配精度更高、性能更好
- ⚡ N-gram 候选预筛选（2-gram 索引）：模糊匹配只比较有交集的题库条目，大幅减少 strSim 调用次数
- ⚡ 长度分桶索引：题目按长度分 5 个桶（≤15/≤30/≤50/≤80/+），匹配时只比较同桶 ±1 范围
- ⚡ requestIdleCallback 分帧结构预留：大批量遍历时可分帧执行，防止卡顿主线程

### 🤖 AI 辅助匹配（新增）
- 题库无命中时，自动调用 DeepSeek / 硅基流动 API 做语义匹配兜底
- 用户自填 API Key（支持硅基流动免费额度），设置区可配置开关

### ☁️ GitHub Gist 云同步（新增）
- ⬆ 一键上传题库到指定 Gist（自动创建/更新）
- ⬇ 一键下载并合并云端题库
- Token 和 Gist ID 在设置区配置，首次上传后 Gist ID 自动保存

### ⌨️ 快捷键支持（新增）
- `Alt+Enter` — 开始答题 / 已运行时提交
- `Alt+S` — 暂停 / 继续
- `Alt+D` — 快速导出题库 JSON
- `Ctrl+Shift+A` — 切换 AI 辅助开关（控制台提示）
- 设置区可全局开关快捷键功能

### 🔔 系统通知（新增）
- 答题完成、提交试卷、答案采集成功、题库云同步完成时发送 `GM_notification` 系统通知

### 📊 答题报告导出（新增）
- JSON 报告：包含版本、URL、时间戳、命中率、按匹配方式统计
- CSV 报告：题目 / 答案 / 匹配方式 / 时间戳，可直接用 Excel 分析
- 每道题的匹配方式（library / rule / ai）均有记录

### 📚 题库浏览增强（升级）
- 支持正则表达式搜索（勾选"正则"复选框）
- 搜索关键词高亮显示
- 按答案筛选（仅 A / 仅 B / 仅 C / 仅 D / 多选）
- 🎲 随机抽查模式（打乱顺序）
- 🗑 清空答题日志

### ⚙️ 配置管理（升级）
- 配置与题库分离导出：题库 JSON/TXT + 独立配置 JSON（含 API Key、Token 等）
- 配置导入：一键恢复账号、AI、云同步等完整设置

### 🐛 修复
- 无

## v4.5.38
- ⚡ strSim 增加公共前缀/后缀裁剪（参考 js-levenshtein），大幅减少 DP 计算量
- ⚡ cleanMap 缓存 cleanedKey，模糊匹配零重复 cleanText 调用
- ⚡ DOM 元素缓存（$c），答题循环中统计卡片查询从 O(每题) 降至 O(1)
- ⚡ renderBrowse 使用 DocumentFragment 批量插入，减少 reflow
- ⚡ 搜索输入 300ms 防抖，避免每次按键触发全量过滤
- ⚡ addBulk 增加 5MB 数据量安全检查，防止 GM_setValue 卡死

## v4.5.37
- ⚡ 性能：缓存合并题库（getMergedDB），避免每题重复 JSON.parse + 对象展开
- ⚡ 性能：预清洗所有 key，精确匹配从 O(n) 降至 O(1)
- ⚡ 性能：strSim 改为一维滚动数组，内存从 O(a×b) 降至 O(min(a,b))
- ⚡ 性能：LibraryManager.count getter 惰性缓存，避免重复 JSON.parse

## v4.5.36
- 优化：增大两行按钮间距（5px → 12px）
- 优化：按钮文字大小改为相对单位（em），随按钮同步缩放

## v4.5.35
- 修复：暂停后按钮文字改为"▶ 继续"，继续后改回"⏸ 暂停"

## v4.5.34
- 优化：按钮布局改用CSS Grid，第一行4列、第二行3列，彻底避免flex换行问题

## v4.5.33
- 优化：倒计时期间按钮文字与答题期间统一，都是"暂停"/"继续"

## v4.5.32
- 优化：倒计时期间暂停按钮文字保持"⏸ 暂停倒计时"不变，不切换为"▶ 继续"

## v4.5.31
- 修复：第一行改为4等分（25%），与4个按钮数量匹配

## v4.5.30
- 优化：暂停按钮始终显示，不再隐藏/显示切换

## v4.5.29
- 移除：停止功能，只保留暂停/继续
  - 运行中：显示"暂停"（黄色），点一下暂停
  - 暂停中：显示"继续"（绿色），点一下恢复答题

## v4.5.27
- 优化：按钮改用 flex-basis 百分比精确计算，两行按钮都精确撑满容器且宽度固定不乱跳

## v4.5.26
- 优化：按钮改为 flex:1 等宽布局，5个一行/3个一行各自平分对齐
- 优化：移除purple按钮特殊字号/内边距，所有按钮样式统一

## v4.5.25
- 优化：统计卡片gray数字/idle状态点/日志文字颜色加深，提升可读性
- 优化：z-index 统一用CSS变量管理（--z-panel / --z-modal）
- 优化：彩色按钮hover时显示微妙背景色，语义更清晰
- 优化：设置折叠区改为max-height动画，展开/收起平滑过渡
- 优化：响应式适配，小屏（≤400px）面板自适应宽度
- 修复：题库表格hover背景对比度提升至12%
- 优化：题库浏览滚动区和日志区统一自定义滚动条样式
- 优化：collapsed圆角写法简化

## v4.5.24
- 修复：收起时标题栏顶部底部都有圆角

## v4.5.22
- 修复：面板收起时保持圆角和阴影

## v4.5.20
- UI更新：采用方案一经典新拟态风格
- 背景色：柔和灰蓝(#e0e5ec)
- 阴影效果：增强的双阴影系统
- 圆角：更大的圆角设计(12-20px)
- 配色：蓝灰色调统一

## v4.5.19
- Word文档导入：同样显示重复题目详情

## v4.5.18
- 导入题库：重复题目会显示出来（显示旧答案→新答案），最多显示10条

## v4.5.17
- 导入时：判断题答案不再转换为 true/false，保持 A/B

## v4.5.16
- 浏览题库：分类筛选改为内置/自定义

## v4.5.15
- 浏览题库：增加题目分类筛选（全部/单选判断/多选）
- 浏览题库：增加页数跳转功能

## v4.5.14
- docx导入：修复同行末尾的选项（如 `赖惠芬A.青色B.黄色`）也能被过滤

## v4.5.13
- docx导入优化：只提取题目（保留出题人）和答案，自动过滤选项和解析

## v4.5.12

## v4.5.11
- docx导入时：多选题答案（如 AB、ABC）自动转换为 A,B,A,B,C 格式

## v4.5.10
- 优化 .docx 导入错误提示：区分 .doc（旧版）和 .docx 格式，明确告知需要转换格式

## v4.5.9
- 面板内容区域支持滚动：当高度不足时自动出现滚动条

## v4.5.8
- 全面圆角化设计：圆角从 2px 升级为 8-12px，视觉更柔和

## v4.5.7
- 主面板 UI 全新设计：采用极简新拟态（Minimalist Neumorphism）风格
- 纯白背景 + 极细双侧阴影，呈现微妙的立体浮雕效果
- 保留所有功能，优化视觉体验

## v4.5.6
- 恢复8方向调整大小功能，但隐藏手柄（保持界面整洁）
- 拖拽标题栏移动，Alt+拖拽标题栏调整大小（四边+四角均可用）

## v4.5.5
- 升级为8方向调整大小（四边+四角手柄，最小 200×150）
- 8个方向均可独立或组合调整面板尺寸

## v4.5.3
- 删除全部拖拽功能（移动面板 + 调节大小），恢复固定位置和尺寸

## v4.5.1
- 优化拖拽功能：添加边界限制、RAF性能优化、触摸支持、视觉反馈

## v4.5.0
- 新增手动调节面板功能：标题栏可拖动移动面板位置，左下角调节手柄可调整面板大小（最小 220×300，最大 600×800）
- 重构拖拽逻辑，标题栏按钮区域不受拖拽影响
- 收起状态下隐藏调节手柄，避免误操作

## v4.4.6
- 修复面板收起/展开不生效的问题：CSS 选择器 `.ata-panel.collapsed` 与 HTML 中的 `#ata-panel` 不匹配
- 新增独立的展开按钮 `#ata-expand-btn`，收起时隐藏收起按钮，展开时显示收起按钮
- 修复倒计时剩余秒数不实时显示：暂停时每秒刷新剩余秒数状态
- 修复倒计时期间无法暂停的根本原因：暂停按钮条件从 `!running` 改为 `!running && !inCountdown`

## v4.4.5
- 修复答题完成后倒计时期间无法暂停的问题：引入 `inCountdown` 标志区分答题阶段和倒计时阶段
- 倒计时阶段暂停按钮保留并变为橙色"⏸ 暂停倒计时"，点击暂停后变为"▶ 继续倒计时"
- 暂停时状态栏显示"⏸ 倒计时已暂停（剩余 Ns）"
- 重置/停止时同步清理所有状态（running/paused/inCountdown/tick）

## v4.4.4
- 新增主面板收起/展开功能：标题栏右侧新增 ▼ 按钮，点击收起主体（仅保留标题栏），再次点击展开

## v4.4.3
- 新增暂停/继续功能：答题过程中可随时点击暂停，循环阻塞等待；继续后从暂停位置恢复答题
- 暂停时控制提交倒计时：暂停倒计时秒数冻结，继续后从剩余秒数继续倒计时
- 面板新增黄色暂停按钮，答题开始时显示，停止/完成/重置时隐藏
- 停止按钮也会同步清理提交倒计时 interval

## v4.4.2
- 修复多选题"没有作答"的问题：根因是 `fill()` 中 `norm(String(answer)).split(',')` 顺序错误，norm() 先吞掉逗号导致 "A,D" 变成 "AD"，无法匹配单个选项；改为先 split 再 norm
- 主面板版本号改为自动读取 `@version` 元数据，不再硬编码

## v4.4.1
- 修复多选题命中但不勾选的问题：checkInput 增加 label 优先触发 + pointerdown/pointerup 事件链 + await 顺序执行防止并行冲突

## v4.4.0
- 主面板 UI 全面升级：暗色主题 + 渐变色 + 统计卡片 + 进度条动画

## v4.3.3
- 重写答案采集逻辑，兼容 MinuteStars 结果页 `.answer-badge.reference` 结构

## v4.3.2
- 修复答案采集误采集用户自己答案的问题

## v4.3.1
- 修复 Tampermonkey 沙箱下 MouseEvent view 报错
