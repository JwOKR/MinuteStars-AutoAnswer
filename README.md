...
- **设置搜索** — 快速定位配置项
- **错题本 + 复习模式** — 艾宾浩斯简化版间隔复习 [1,3,7,15,30] 天，独立弹窗管理
- **语义去重增强** — N-gram + Jaro-Winkler 检测，相似度 > 0.88 自动跳过入库
- **正确率趋势图** — Canvas 实时绘制最近 50 题命中率折线
- **答案来源标注** — 最近答题记录显示彩色圆点（蓝=题库，黄=规则，绿=AI，灰=未匹配）
